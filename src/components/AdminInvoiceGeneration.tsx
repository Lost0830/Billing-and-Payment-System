...
interface AdminInvoiceGenerationProps {
...
export function AdminInvoiceGeneration({ onNavigateToView }: AdminInvoiceGenerationProps) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const plist = Array.isArray(pRes) ? pRes : (pRes as any)?.data || [];
      ...
      try {
        const dyn = (DiscountService as any).getDiscounts?.() || [];
  ...
  const calculateTax = () => {
    const subtotal = calculateSubtotal();
    const taxableAmount = invoiceItems.filter(item => isTaxableItem(item)).reduce((sum, item) => sum + item.amount, 0);
  ...
  const handleGenerateInvoice = async () => {
    ...
    try {
      ...
      // Try to get friendly patient ID before saving
      const selectedPatient = patients.find((p: any) => getInternalPatientKey(p) === newInvoice.patientId);
      if (selectedPatient) {
        (invoice as any).patientNumber = getDisplayPatientId(selectedPatient) || newInvoice.patientId;
      }
      ...
      if (res.data.success) {
        ...
        // Dispatch notification event for cashier to see
        try {
          const notificationData = {
            invoiceNumber: saved.number || saved.invoiceNumber || invoice.number,
            patientName: saved.patientName || invoice.patientName,
            total: saved.total || invoice.total,
            timestamp: new Date().toISOString()
          };
          
          // Store in localStorage
          localStorage.setItem('lastAdminInvoiceCreated', JSON.stringify(notificationData));
          
          // Also dispatch event
          window.dispatchEvent(new CustomEvent('admin-invoice-created', { 
        ...
        try {
          ...
          try { window.dispatchEvent(new CustomEvent('billing-updated', { detail: { record } })); } catch (e) { }
        } catch (e) {
  ...
  const confirmDeleteInvoice = async () => {
    ...
    try {
      const res: any = await axios.post(`${API_BASE}/archive/invoices/${invoiceToDelete}/archive`, { archivedBy: 'admin' });
      if (res?.data?.success) {
  ...
  return (
    <div className="space-y-6">
      ...
      {showCreateForm && (
        <Card>
          ...
          <CardContent className="p-6 space-y-6">
                ...
                <Popover open={openPatientCombobox} onOpenChange={setOpenPatientCombobox}>
                  <PopoverTrigger asChild>
                    ...
                    >
                      {selectedPatientId
                        ? (() => {
                            const p = patients.find((x:any) => getInternalPatientKey(x) === selectedPatientId || x.id === selectedPatientId || x._id === selectedPatientId || x.patientId === selectedPatientId);
                            const pid = p ? (getDisplayPatientId(p) || getInternalPatientKey(p)) : selectedPatientId;
                            const name = p?.name || "Selected Patient";
                            return `${name} (${pid})`;
                          })()
            ...
            {/* Service Items */}
            <div>
              <h3 className="mb-4 text-lg">Add Services</h3>
                    ...
                    <SelectContent>
                      {(() => {
                        ...
                        const legacyItems = DISCOUNT_OPTIONS.filter(lo => {
                          const base = lo.label.split('(')[0].trim().toLowerCase();
      ...
      <Dialog open={showInvoiceDetails} onOpenChange={setShowInvoiceDetails}>
        <DialogContent className="max-w-[600px] w-full max-h-[85vh] overflow-auto receipt-dialog">
          <DialogHeader>
            <DialogTitle>Invoice Details</DialogTitle>
          </DialogHeader>
          {selectedInvoice && (
            <div id="invoice-receipt-print" className="overflow-y-auto">
              <TransactionRecord
          ...
          <div className="flex justify-end space-x-2 mt-4">
            <Button variant="outline" size="sm" onClick={() => setShowInvoiceDetails(false)}>
              Close
            </Button>
            <Button size="sm" onClick={handlePrintReceipt} className="bg-[#358E83] hover:bg-[#358E83]/90 text-white">
      ...
      <Card>
        <CardHeader className="bg-[#358E83] text-white">
          ...
          <CardDescription className="text-white/80">
            Manage invoices created by admin
          </CardDescription>
        </CardHeader>
      ...
      <AlertDialog open={showCreateConfirm} onOpenChange={setShowCreateConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Confirm Create Invoice?</AlertDialogTitle>
            <AlertDialogDescription>
              This will create an invoice for {newInvoice.patientName || 'the selected patient'}.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between"><span>Subtotal</span><span>₱{calculateSubtotal().toLocaleString('en-PH', { minimumFractionDigits: 2 })}</span></div>
            <div className="flex justify-between text-green-700"><span>Discount</span><span>-₱{calculateDiscount().toLocaleString('en-PH', { minimumFractionDigits: 2 })}</span></div>
            {calculateTax() > 0 && (<div className="flex justify-between text-blue-700"><span>VAT</span><span>₱{calculateTax().toLocaleString('en-PH', { minimumFractionDigits: 2 })}</span></div>)}
            <div className="border-t my-1"></div>
            <div className="flex justify-between font-semibold"><span>Total</span><span>₱{calculateTotal().toLocaleString('en-PH', { minimumFractionDigits: 2 })}</span></div>
          </div>
          <AlertDialogFooter>
